...
if echo "$COMMAND" | grep -qiE '(^|[[:space:]]|;|&&|\|)(echo|printf|cat)[[:space:]]+.*(\$\{?[A-Za-z_]*|[A-Za-z_]*)(KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|AUTH)'; then
...
if echo "$COMMAND" | grep -qE 'cat[[:space:]]+(\.env\.secrets|secrets/\.env|~/dotfiles/secrets/)'; then
